...
const Application = {
  ...
  async findByUser(userId) {
    const [rows] = await getPool().query(
      `SELECT a.id, a.status, a.applied_date, j.id AS job_id, j.title, j.company, j.location, j.salary
       FROM applications a
  ...
  async findByCompany(companyUserId) {
    const [rows] = await getPool().query(
      `SELECT a.id, a.status, a.applied_date, a.job_id, j.title, j.company, j.type,
